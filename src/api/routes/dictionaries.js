...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const dictionaries = await dictionaryService.listDictionaries(type, { search });
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const dictionary = await dictionaryService.createDictionary(type, { raw, pinyin, abbreviation });
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const dictionary = await dictionaryService.updateDictionary(id, { raw, pinyin, abbreviation });
    if (!dictionary) {
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await dictionaryService.deleteDictionary(id);
    if (!deleted) {
      return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: '词典条目不存在' } });
    }
