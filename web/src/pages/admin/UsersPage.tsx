...
import { adminApi, type PasswordResetResult } from '../../api';
...
function UserDetailDrawer({ userId, currentUserId, onClose, onChanged }: { userId: string; currentUserId?: string; onClose: () => void; onChanged: () => void }) {
  ...
  const execute = async () => {
    if (!action) return;
    const { password, reason } = await form.validateFields();
    setWorking(true);
    try {
...
export default function UsersPage() {
  ...
  const execute = async () => {
    if (!action) return;
    const { password, reason } = await actionForm.validateFields();
    setWorking(true);
    try {
      await adminApi.elevate(password);
  ...
  return <>{contextHolder}<PageHeader title={t('admin.userManagement')} actions={<Button icon={<ReloadOutlined />} onClick={state.reload}>{t('common.refresh')}</Button>} />
    <Card className="surface table-card" title={<Space wrap><Input.Search allowClear prefix={<SearchOutlined />} placeholder={t('common.search')} value={input} onChange={(event) => setInput(event.target.value)} onSearch={() => setQuery(input.trim())} style={{ width: 280 }} /><Select allowClear placeholder={t('common.role')} value={role} onChange={setRole} style={{ width: 150 }} options={[{ value: 'admin', label: t('role.admin') }, { value: 'user', label: t('role.user') }]} /></Space>}>
      <AsyncContent loading={state.loading} error={state.error} empty={!state.loading && !state.data?.items.length} onRetry={state.reload}>
        <Table<User> rowKey="id" dataSource={state.data?.items ?? []} scroll={{ x: 830 }} pagination={{ current: page, pageSize, total: state.data?.total, showSizeChanger: true, onChange: (next, size) => { setPage(next); setPageSize(size); } }} columns={[
