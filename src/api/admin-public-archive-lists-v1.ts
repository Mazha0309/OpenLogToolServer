...
import { normalizePublicArchiveAlias, publicArchiveAliasDisplay } from '../public-archives/model';
...
export function createAdminPublicArchiveListsV1Router(dependencies: AdminPublicArchiveListsV1Dependencies = {}): Router {
  ...
  router.put('/public-archive-lists/:listId/alias', (req: V1AuthRequest, res, next) => { try {
    const body = requireJsonObject(req.body); rejectUnknownKeys(body, ['alias']); const displayAlias = publicArchiveAliasDisplay(requireString(body, 'alias', { max: 63 })); const alias = normalizePublicArchiveAlias(displayAlias);
    res.json({ data: assignArchiveListAlias(database(), req.params.listId, { userId: req.auth!.userId, role: 'admin' }, alias, displayAlias) });
